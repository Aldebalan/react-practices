...
01. webpack은 작게 분리된 많은 애플리케이션의 모듈(js, css, images, font)를 '의존성 분석'하여 하나의 JS 파일르 번들링 하는것
02. 최종 결과물인 하나의 JS 파일을 번들 (bundle)이라고 부르고 묶는 작업을 번들링(bundling)이라 한다.
03. 번들링은 단순히 모듈을 하나의 파일로 묶는 작업을 의미하지 않는다.
04. 빌드 작업 (js 모듈)
    ...
    4) 난독화/압축(uglify) 작업
    5) 번들링
05. js 모듈 뿐만 아니라 다양한 asset(images, css, sass/scss, font)들도 모듈로 취급한다.
06. 설치
